...
import { usernameLoginId, ChangeAdminSponser } from '@/app/redux/adminMasterSlice';
import { toast } from 'react-toastify';

const ChangeSponser = () => {
  const dispatch = useDispatch();
  const { usernameData, loading, error, sponserData } = useSelector((state) => state.adminMaster);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await dispatch(ChangeAdminSponser({ authLogin, sponsorAuthLogin }));
      const payload = result.payload;
      const statusCode = payload?.statusCode || (payload?.data && payload.data[0]?.statusCode);
      const message = payload?.message || (payload?.data && payload.data[0]?.message);
      ...
      toast.error('Error changing sponsor: ' + (error.message || 'Unknown error'));
    }
  };
  ...
  return (
    <div className="flex items-center justify-center mt-20">
      <div className="w-full max-w-4xl p-8 bg-white shadow-2xl rounded-2xl">
        <h2 className="mb-8 text-2xl font-bold text-center text-black">Change Sponser</h2>
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          {userName && (
            <div className="flex flex-col gap-4 mt-4 md:flex-row">
              <div className="flex-1">
                <label className="block mb-1 text-sm font-semibold text-gray-700">Sponsor User ID :</label>
                <input
                  type="text"
                  className="w-full px-4 py-3 text-base bg-gray-100 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-300"
