...
//   const chainLabel = chainNames[chainId] || chainId || '-';
...
//               <span
//                 className="px-2 py-1 font-mono text-xs bg-white border rounded shadow-sm"
//                 title={account}
//               >
...
//                 className="inline-flex items-center gap-1 px-2 py-1 text-xs border rounded hover:bg-orange-100"
//                 title="Copy address"
//               >
//                 <FaCopy className="w-3 h-3" />
//                 Copy
...
//                       <button
//                         className="px-3 py-1 font-semibold text-white bg-green-500 rounded hover:bg-green-600"
//                         onClick={() => handleApproveUSDTClick(row.AuthLogin)}
//                       >
//                         Approve USDT
//                       </button> 
//                     </td>
...
//   const [approvePopupOpen, setApprovePopupOpen] = useState(false);
//   const [rejectPopupOpen, setRejectPopupOpen] = useState(false);
//   const [approveUSDTPopupOpen, setApproveUSDTPopupOpen] = useState(false);
//   const [selectedAuthLoginId, setSelectedAuthLoginId] = useState(null);
...
//   const handleApproveUSDTClick = (row) => {
//     setSelectedRowData(row);
//     setSelectedAuthLoginId(row.AuthLogin);
//     setWalletAddress(row.walletAddress || '');
//     setTransactionHash(row.Wallet || '');
//     setApproveUSDTPopupOpen(true);
//   };
...
//     if (selectedRowData && account.trim() && selectedRowData.Wallet.trim()) {
//       try {
//         await dispatch(updateIncomeWalletAdressUSDT({
//           authLoginId: selectedAuthLoginId,
//           debit: selectedRowData.debit,
//           wallet:selectedRowData.Wallet ,
//           transHash: account
//         }));
        
//         setApproveUSDTPopupOpen(false);
//         setSelectedAuthLoginId(null);
...
//               {paginatedRows.length === 0 ? (
//                 <tr>
//                   <td colSpan={12} className="py-10 text-lg text-center text-gray-400">No Approve Withdrawal Requests Found</td>
//                 </tr>
//               ) : (
//                 paginatedRows.map((row, idx) => (
//                   <tr key={idx} className={idx % 2 === 0 ? 'bg-blue-50 hover:bg-blue-100 transition' : 'bg-white hover:bg-blue-50 transition'}>
...
//                     <td className="px-4 py-2 text-sm text-center">
//                       <button
//                         className="px-3 py-1 font-semibold text-white bg-purple-500 rounded hover:bg-purple-600"
...
//       {/* Approve USDT Popup Modal */}
//       {approveUSDTPopupOpen && (
//         <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
//           <div className="w-full max-w-md p-6 bg-white rounded-lg shadow-lg">
//             <div className="mb-4 text-lg font-semibold text-gray-800">
//               Approve USDT Transaction for AuthLoginID <span className="text-blue-600">{selectedAuthLoginId}</span>
//             </div>
            
//             <div className="mb-4">
//               <label className="block mb-2 text-sm font-medium text-gray-700">Wallet Address</label>
//               <input
//                  value={selectedRowData.Wallet || ''}
...
//             <div className="mb-4">
//               <label className="block mb-2 text-sm font-medium text-gray-700">Transaction Hash</label>
//               <input
//                 type="text"
//                 className="w-full p-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
//                 value={account || ''}
...
//             <div className="mb-4">
//               <label className="block mb-2 text-sm font-medium text-gray-700">Amount (Debit)</label>
//               <input
//                 type="text"
//                 className="w-full p-2 bg-gray-100 border border-gray-300 rounded-md"
//                 value={selectedRowData?.debit || ''}
//                 disabled
//               />
//             </div>
...
//                 className={`px-4 py-2 font-semibold text-white rounded ${!account.trim() || !transactionHash.trim() ? 'bg-gray-400 cursor-not-allowed' : 'bg-purple-600 hover:bg-purple-700'}`}
//                 disabled={!account.trim() || !transactionHash.trim()}
//               >
//                 Submit
//               </button>
//               <button
...
const WithdrawalRequest = () => {
  ...
  const fetchWalletBalance = async (accountAddress) => {
    ...
    try {
      ...
      const balanceInBnb = parseInt(balance) / 1e18;
      setWalletBalance(balanceInBnb.toFixed(4));
      return balanceInBnb;
    } catch (error) {
  ...
  const sendUSDTTransaction = async (toAddress, amount) => {
    ...
    try {
      ...
      const transactionParameters = {
        ...
        data: 
          // USDT transfer method signature + recipient address + amount
          '0xa9059cbb' + 
 ...
 const formatAddress = (addr) => (addr ? addr : '-');
  ...
  const handleApproveUSDTClick = async (row) => {
    ...
    const currentBalance = await fetchWalletBalance(account);
      ...
      // toast.error(`Insufficient BNB ! You need at least 0.001 BNB`);
      // return;
    // }
    ...
    try {
      setIsSending(true);
      // toast.info('Please confirm the transaction in MetaMask...', { autoClose: 5000 });

      const txHash = await sendUSDTTransaction(row.Wallet, row.debit);
