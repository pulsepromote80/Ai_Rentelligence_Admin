  ...
  const handleOtpVerify = async () => {
    ...
    if (otpValue === HARD_OTP) {
      try {
        ...
        setOtpMessage("Login failed. Please try again.");
      }
    } else {
      ...
      <div className="flex flex-col w-full max-w-4xl bg-white shadow-2xl md:flex-row xl:max-w-4xl rounded-2xl overflow-auto">
        ...
        <div className="relative flex flex-col justify-center w-full p-4 text-white md:w-2/5 xs:p-6 sm:p-8 lg:p-10 bg-gradient-to-br from-indigo-600 to-purple-700">
          ...
          <div className="relative z-10">
            <h1 className="mb-3 text-xl font-bold xs:text-2xl sm:text-3xl md:text-4xl">Welcome Back!</h1>
            <p className="mb-6 text-xs leading-relaxed text-indigo-200 xs:text-sm sm:text-base">
            ...
            <div className="relative flex items-center justify-center h-24 xs:h-32 sm:h-40 md:h-56">
              <div className="flex items-center justify-center w-20 h-20 xs:w-24 xs:h-24 sm:w-32 sm:h-32 md:w-40 md:h-40 bg-white/20 rounded-3xl backdrop-blur-sm rotate-12">
                <svg xmlns="http://www.w3.org/2000/svg" className="w-8 h-8 text-white transform xs:w-12 xs:h-12 sm:w-16 sm:h-16 md:w-20 md:h-20 -rotate-12" fill="currentColor" viewBox="0 0 24 24">
        ...
        {/* Right side - Form */}
        <div className="flex flex-col justify-center w-full p-4 bg-white md:w-3/5 xs:p-6 sm:p-8 lg:p-10">
          <div className="mb-6 text-center">
            <h2 className="text-xl font-bold text-gray-800 xs:text-2xl sm:text-3xl">Admin Login</h2>
          </div>

          <form onSubmit={handleLogin} className="space-y-5">
            {error && (
              <div className="flex items-center p-3 text-sm text-red-700 rounded-lg bg-red-50">
            ...
            {/* Username */}
            <div>
              <label htmlFor="username" className="block mb-2 text-xs font-medium text-gray-700 xs:text-sm">Username</label>
              <div className="relative">
                <FaUser className="absolute w-3 h-3 text-gray-500 -translate-y-1/2 left-3 xs:left-4 top-1/2 xs:w-4 xs:h-4" />
                <input
                  id="username"
                  type="text"
              ...
              {errors.username && <p className="mt-2 text-xs text-red-600 xs:text-sm">{errors.username}</p>}
            </div>

            {/* Password */}
            <div>
              <label htmlFor="password" className="block mb-2 text-xs font-medium text-gray-700 xs:text-sm">Password</label>
              <div className="relative">
                <FaLock className="absolute w-3 h-3 text-gray-500 -translate-y-1/2 left-3 xs:left-4 top-1/2 xs:w-4 xs:h-4" />
                <input
                  id="password"
                ...
                <button
                  ...
                  className="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-500 xs:pr-4 hover:text-indigo-600"
                >
                  {showPassword ? <EyeSlashIcon className="w-4 h-4 xs:w-5 xs:h-5" /> : <EyeIcon className="w-4 h-4 xs:w-5 xs:h-5" />}
                </button>
              </div>
              {errors.password && <p className="mt-2 text-xs text-red-600 xs:text-sm">{errors.password}</p>}
            </div>
              ...
              <div className="mt-4 space-y-3">
                <div className="flex flex-col space-y-3 sm:flex-row sm:items-center sm:space-x-2 sm:space-y-0">
                  <input
                    ...
                    className={`flex-1 px-4 py-2 xs:py-3 border rounded-xl focus:ring-2 focus:ring-indigo-500 border-gray-300 text-center text-sm xs:text-base ${!otpSent || verifyingOtp ? 'bg-gray-100 cursor-not-allowed' : ''}`}
                  />
                  <button
                    type="button"
                    onClick={handleOtpSend}
                    disabled={otpSent || verifyingOtp}
                    className={`w-full sm:w-auto px-4 py-2 xs:py-3 text-white rounded-xl transition-colors duration-200 text-sm xs:text-base ${otpSent || verifyingOtp ? 'bg-gray-400 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-700'}`}
                  >
                    Send OTP
                  </button>
                </div>
                {otpMessage && (
                  <p className={`text-xs xs:text-sm mt-1 ${otpMessage.includes('successfully') || otpMessage.includes('sent') ? 'text-green-600' : 'text-red-600'}`}>
                    {otpMessage}
                  </p>
                )}
                ...
                {otpSent && (
                  <button
                    ...
                    className="w-full py-2 xs:py-3 mt-3 text-white bg-gradient-to-r from-indigo-600 to-purple-600 rounded-xl hover:from-indigo-700 hover:to-purple-700 transition-all transform hover:-translate-y-0.5 text-sm xs:text-base flex items-center justify-center disabled:opacity-70 disabled:cursor-not-allowed disabled:transform-none min-h-[42px]"
                  >
                    {verifyingOtp ? (
            ...
            {!showOtp && (
              ...
              >
                {loading ? (
                  ...
                  <>
                    <span>LOG IN</span>
                    <FaArrowRight className="w-3 h-3 ml-2 xs:w-4 xs:h-4" />
                  </>
                )}
              </button>
            )}
